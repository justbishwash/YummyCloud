            ...
            <div className="flex gap-2 items-start">
              ...
              <button
                onClick={async () => {
                  ...
                  if (!partnerId) return;
                  setUpdatingStatus('assign');
                  try {
                    await api.assignDelivery(id, partnerId, order._trackingCode || null, order._skipSms || false);
                    await api.updateOrderStatus(id, 'on_the_way');
                    const res = await api.getOrder(id);
                    setOrder(res.order);
                  } catch (err) { alert(err.message); }
                  finally { setUpdatingStatus(null); }
                }}
                disabled={updatingStatus === 'assign' || !(order._selectedPartner || order.delivery_partner_id)}
                className="bg-primary text-white px-3 py-2 rounded-lg text-xs font-medium disabled:opacity-50 whitespace-nowrap flex items-center gap-1.5"
              >
                {updatingStatus === 'assign' ? <span className="w-3.5 h-3.5 border-2 border-white/30 border-t-white rounded-full animate-spin" /> : null}
                {updatingStatus === 'assign' ? 'Assigning...' : 'Assign & Dispatch'}
              </button>
            </div>
            <label className="flex items-center gap-1.5 mt-1.5 cursor-pointer">
              <input type="checkbox" checked={order._skipSms || false} onChange={(e) => setOrder((prev) => ({ ...prev, _skipSms: e.target.checked }))} className="rounded w-3.5 h-3.5" />
              <span className="text-[10px] text-gray-500">No tracking SMS</span>
            </label>
          </div>
        </div>
      )}
